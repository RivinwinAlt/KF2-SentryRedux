Class ST_Upgrades_FireTurret extends ST_Upgrades_Base;

var bool bFireDoT, bRegen;
	...
	for(i = 1; i < TotalUpgrades; i++) // TotalUpgrades is always the last upgrade enum value, can be used as int
	{
		...
		if(HasUpgrade(i))
		{
			...
			switch(i)
			{
				...
				TurretOwner.DamageTypes[EPrimaryFire]=class'KFDT_Fire';
				bFireDoT=True;
					break;
				case EUpAutoRepair:
				bRegen=true;
...
// It is best practice to store a value/second in BValue and then multiply it by TimerPeriod to get Value or FValue when used here, this allows TimerPeriod to change without invalidating the upgrade
	if (bRegen)
		TurretOwner.Health = Min(TurretOwner.Health + UpgradeInfos[EUpAutoRepair].Value, TurretOwner.HealthMax);
}
...
{
	if(bFireDoT && HitActor.IsA(KFPawn))
		KFPawn(HitActor).ApplyDamageOverTime(InDamage, TurretOwner.Controller, class'KFDT_Fire_FlareGunDoT');
}
