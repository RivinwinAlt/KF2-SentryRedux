...
simulated function PostBeginPlay()
{
	...
	{
		AmmoLevel[0] = ContentRef.MaxAmmoCount[0] * ContentRef.StartingAmmoMultiplier;
		AmmoLevel[1] = ContentRef.MaxAmmoCount[1] * ContentRef.StartingAmmoMultiplier;
		bRecentlyBuilt = true;
		SentryWorth = ContentRef.LevelCfgs[0].Cost;
		Health = ContentRef.LevelCfgs[0].Health;
		HealthMax = ContentRef.LevelCfgs[0].Health;
		if(Controller == None)
			SpawnDefaultController();
		UpdateRandomDamage();
	}

	SightRadius = ContentRef.BaseSightRadius;
	AccuracyMod = ContentRef.BaseAccuracyMod;
...
{
	local int DamRange;
	if(ContentRef.bRandomDamage)
	{
		DamRange = ContentRef.LevelCfgs[PowerLevel].Damage * (ContentRef.RandomDamagePercent / 100);
	} else {
		DamRange = 0;
	}
	RandDamMin = ContentRef.LevelCfgs[PowerLevel].Damage - DamRange;
	RandDamMax = ContentRef.LevelCfgs[PowerLevel].Damage + DamRange;
}
...
function CheckUserAlive() // Check if owner player disconnects from server.
{
	...
	{
		Health -= ContentRef.HealthLostNoOwner;
		if(Health <= 0)
			KilledBy(None);
	}
}
...
final function bool FindNewOwner()
{
	...
	foreach CurrentUsers(N)
	{
		if(N.PlayerOwner != None && N.PlayerOwner.Pawn != None && N.PlayerOwner.Pawn.IsAliveAndWell())
		{
            ...
            if (KFW != None && KFW.NumTurrets < ContentRef.MaxTurretsPerUser)
            {
...
simulated function string GetInfo()
{
	...
	return "Owner: "$(PlayerReplicationInfo != None ? PlayerReplicationInfo.PlayerName : "None")$" ("$F$"% HP)";
}
simulated function string GetAmmoStatus()
{
	...
	if(PowerLevel == 2)
		S = " - "$AmmoLevel[1]$"/"$ContentRef.MaxAmmoCount[1];
	return AmmoLevel[0]$"/"$ContentRef.MaxAmmoCount[0]$S;
}
...
simulated function SetLevelUp()
{
	if(WorldInfo.NetMode != NM_Client)
	{
		++PowerLevel;
		bRecentlyBuilt = true;
		HealthMax = ContentRef.LevelCfgs[PowerLevel].Health;
...
simulated final function bool CanUpgrade(byte Index)
{
	...
	case ETU_AmmoSMGBig:
		...
		return AmmoLevel[0] < ContentRef.MaxAmmoCount[0];
	case ETU_AmmoMissiles:
	case ETU_AmmoMissilesBig:
	
		return PowerLevel == 2 && AmmoLevel[1] < ContentRef.MaxAmmoCount[1];
	}
	return false;
}
...
final function ApplyUpgrade(byte Index)
{
	if(Index >= ETU_AmmoSMG)
	{
		...
		{
		case ETU_AmmoSMG:
			AmmoLevel[0] = Min(AmmoLevel[0] + 100, ContentRef.MaxAmmoCount[0]);
			break;
		case ETU_AmmoSMGBig:
			AmmoLevel[0] = Min(AmmoLevel[0] + 500, ContentRef.MaxAmmoCount[0]);
			break;
		case ETU_AmmoMissiles:
			AmmoLevel[1] = Min(AmmoLevel[1] + 10, ContentRef.MaxAmmoCount[1]);
			break;
		case ETU_AmmoMissilesBig:
			AmmoLevel[1] = Min(AmmoLevel[1] + 50, ContentRef.MaxAmmoCount[1]);
			break;
...
simulated final function SetUpgrades()
{
	if(HasUpgradeFlags(ETU_IronSightB))
		AccuracyMod = ContentRef.BaseAccuracyMod / 4.0f;
	else if(HasUpgradeFlags(ETU_IronSightA))
		AccuracyMod = ContentRef.BaseAccuracyMod / 2.0f;
	else AccuracyMod = ContentRef.BaseAccuracyMod;
	
	if(WorldInfo.NetMode != NM_Client)
	{
		SightRadius = ContentRef.BaseSightRadius;
		if(HasUpgradeFlags(ETU_EagleEyeB))
			SightRadius = ContentRef.BaseSightRadius * 1.3f;
		else if(HasUpgradeFlags(ETU_EagleEyeA))
			SightRadius = ContentRef.BaseSightRadius * 1.6f;
...
simulated function TurretSetFiring(bool bFire, optional bool bInstant)
{
	...
	if(bFire)
	{
		...
		FireShot();
		SetTimer(ContentRef.LevelCfgs[PowerLevel].RoF, true, 'FireShot');
	}
	else
	{
	...
	if(HP > 800)
	{
		...
		if(Proj != None)
		{
			if(HasUpgradeFlags(ETU_HomingMissiles))
				Proj.AimTarget = T;
			Proj.Damage = ContentRef.MissileHitDamage;
			Proj.ExplosionTemplate.Damage = ContentRef.MissileHitDamage;
			Proj.Init(vector(R));
			Proj.InstigatorController = OwnerController != None ? OwnerController : Controller;
			Proj.Speed = ContentRef.MissileSpeed;
		}
		 --AmmoLevel[1];
	}
}
...
simulated function TraceFire()
{
	...
	if(A != None)
	{
		...
		if(WorldInfo.NetMode != NM_Client)
		{
			...
			A.TakeDamage(RandomizedDam, (OwnerController != None ? OwnerController : Controller), HL, Dir * 10000.f, class'KFDT_Ballistic', H, Self);
			...
			if(OwnerController != None && Pawn(A) != None && Pawn(A).Controller != None)
				Pawn(A).Controller.NotifyTakeHit(Controller, HL, 14, class'KFDT_Ballistic', Dir); // Make enemy AI aggro the turret
		}
		else A.TakeDamage(RandomizedDam, Controller, HL, Dir * 10000.f, class'KFDT_Ballistic', H, Self); // changed from flat 14 damage in client
	}
	else HL = End;
...
function AutoRepairTimer()
{
	...
	Health = Min(Health + ContentRef.HealthRegenRate, HealthMax);
	if(Health >= HealthMax)
	{
		AutoRepairState = 0;
...
function AdjustDamage(out int InDamage, out vector Momentum, Controller InstigatedBy, vector HitLocation, class<DamageType> DamageType, TraceHitInfo HitInfo, Actor DamageCauser)
{
	if(class<KFDT_Sonic>(DamageType) != None)
		InDamage *= ContentRef.SonicDamageMultiplier;
