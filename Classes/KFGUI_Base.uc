...
var() bool bDisabled, bClickable, bCanFocus, bCenterCoords;
...
function ComputeCoords()
{
	if(bCenterCoords)
	{
		...
		CompPos[0] = (XPosition * InputPos[2]) + InputPos[0] + ((InputPos[2] - CompPos[2]) / 2.0f);
		CompPos[1] = (YPosition * InputPos[3]) + InputPos[1] + ((InputPos[3] - CompPos[3]) / 2.0f);
	}
	else
	{
		CompPos[2] = XSize*InputPos[2];
...
{
	return (bClickable ? 0 : 1); // ? `PEN_BLACK : `PEN_WHITE); // Depricated, reimplement before use
}
...
function bool NotifyInputKey( int ControllerId, name Key, EInputEvent Event, float AmountDepressed, bool bGamepad )
{
	if( bEnableInputs )
	{
		...
		case 'MouseScrollUp':
			if( Event==IE_Pressed )
				ScrollMouseWheel(Key=='MouseScrollUp');
			return true;
		}
	}
